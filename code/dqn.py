...
class GridWorld():
    ...
    def get_dropout_mask(self):
        m = np.random.rand(self.observable[0]*self.observable[1]).reshape(self.observable)
...
class ExperienceReplay():
    ...
    def sample(self, B):
        ...
        return (self.S_memory[indices], self.A_memory[indices], self.R_memory[indices],
                self.S_prime_memory[indices], self.T_memory[indices], None)


class TemporalExperienceReplay():
    def __init__(self, T, state_shape, max_queue_size, stateful=True, memory=0,
                 dtype="uint8"):
        self.max_queue_size = max_queue_size
        # Initiate S and S' memories
        self.T = T
    ...
    def sample(self, B):
        ...
        if self.stateful:
            ...
            if self.memory > 0:
                M =  self.S_memory[self.sampling_indices, self.sampling_timestep-1-self.memory:self.sampling_timestep-1]
                if M.shape[1] < self.memory:
                    M  = pad_empty_grids(M, self.memory)
                out[-1] = M
        else:
            out =  (self.S_memory[self.sampling_indices, :],
                    self.A_memory[self.sampling_indices, :],
                    self.R_memory[self.sampling_indices, :],
                    self.S_prime_memory[self.sampling_indices, :],
                    self.T_memory[self.sampling_indices, :],
                    None)
...
class DQNAgent():
    ...
    def __init__(self, environment, net, replay, batch_size, target_net=None, gamma=0.95,
                 epsilon=0.8, target_net_patience=10, nitermax=10000, nb_of_actions=4, stack=1):
        self.E = environment
        self.DQN = net
        self.replay = replay
        self.B = batch_size
    ...
    def qlearning_generator(self, replay=None, batch_size=None, verbose=True, seed=123):
        ...
        while True:
            ...
            for it in range(self.nitermax):
                ...
                S_batch, A_batch, R_batch, S_prime_batch, T_batch, M_batch = replay.sample(batch_size)

                self.on_batch_start(it, S_batch)

                # Preprocess states and get Q(S,:)
                if self.stack > 1:
                    S_prime_batch = np.concatenate([M_batch[:,1:], S_batch[:,None], S_prime_batch[:,None]], axis=1)
                    S_batch = np.concatenate([M_batch, S_batch[:,None]], axis=1)
                ...
                # Preprocess memories
                if M_batch is not None or self.stack > 1:
                    M_batch = self.preprocess(M_batch)
                # Get target
                ...
                if replay.memory == 0 or self.stack > 1:
                    yield S_batch, np.array(target, "float32")
                else:
                    yield [S_batch, M_batch], np.array(target, "float32")
                ...
                if self.E.is_terminal():
                    ...
                    if not replay.memory == 0 and self.stack == 1:
                        #print(M_batch[0:1, 1:].shape)
                        #print(S_batch[0:1, None].shape)
                        M = np.concatenate([M_batch[0:1, 1:], S_batch[0:1, None]], axis=1)
                    else:
                        M = None
                    A_prime =  self.sample_next_action(S_prime_batch[0:1], Q_prime_batch[0:1], it, M)
    ...
    def preprocess(self, S_batch):
        if self.stack == 1:
            ...
            S = np.array([preprocess_grids(S_batch[i,:]) for i in range(S_batch.shape[0])])
            S = S.reshape((S.shape[0], S.shape[1]*S.shape[2], S.shape[3], S.shape[4]))
            return S


    def compute_target_and_Qprime(self, S_batch, S_prime_batch, A_batch, R_batch, T_batch, M_batch):
    ...
    def sample_next_action(self, S, Q=None, it=None, M_batch=None):
        if Q is None:
...
class StatefulRDQNAgent(DQNAgent):
    ...
    def __init__(self, environment, net, net_copy, replay, batch_size, target_net, flatten=True, gamma=0.95, epsilon=0.8,
...
class RDQNAgent(DQNAgent):
    ...
    def __init__(self, environment, net, stateful_net, replay, batch_size, target_net, flatten=True, gamma=0.95, epsilon=0.8,
                 target_net_patience=10, nitermax=10000, nb_of_actions=4):
        self.T = replay.T
        self.flatten = flatten
        self.G = np.array([gamma**(k) for k in range(self.T)], "float32")[None]
        self.stateful_DQN = stateful_net
        DQNAgent.__init__(self, environment, net, replay, batch_size, target_net, gamma,
                          epsilon, target_net_patience, nitermax, nb_of_actions)
    ...
    def compute_target_and_Qprime(self, S_batch, S_prime_batch, A_batch, R_batch, T_batch, M_batch):
        ...
        #      - target[selected action] = R + gamma*max(Q(S',:))
        #      - target[other actions] = Q(S,:) (i.e. the loss is 0 for non-selected actions)
        cum_rewards = np.cumsum((R_batch*self.G)[:, ::-1], axis=1)[:, ::-1]
        A_batch_oh = temporal_one_hot(A_batch, self.A)
        target = (cum_rewards + self.gamma*T_batch[:,-1:]*np.max(Q_prime_batch[:,-1], axis=1)[:,None])[:,:,None]*A_batch_oh + Q_batch*(1.-A_batch_oh)
        return target, Q_prime_batch
    ...
    def sample_next_action(self, S, Q=None, it=None, M_batch=None):
        if self.replay.temporal_idx == 0:
            it = self.T-1
        else:
            it = self.replay.temporal_idx - 1
        Q = self.stateful_DQN.predict(S[:,it:it+1])
        Q = Q.flatten()
        return epsilon_greedy(Q, self.epsilon)

    def preprocess(self, S_batch):
        S = np.concatenate([preprocess_grids(S_batch[:,i]) for i in range(self.T)], axis=1)
        if self.flatten:
            return S.reshape(self.B, self.T, -1)
        else:
            return S

    def on_epoch_start(self, patience):
        # Reset states of statefull lstm
        self.stateful_DQN.reset_states()
        # Update the weights of 'target_net'
        if patience == self.target_DQN_patience:
            self.target_DQN.set_weights(self.DQN.get_weights())
            return 0
...
class StatefulDRMQNAgent(StatefulRDQNAgent):
    ...
    def compute_target_and_Qprime(self, S_batch, S_prime_batch, A_batch, R_batch, T_batch, M_batch):
        ...
        #print(Q_batch[0].flatten())
        M_prime_batch = np.concatenate([M_batch[:, 1:], S_batch[:, None]], axis=1)
        Q_prime_batch = self.target_DQN.predict([S_prime_batch, M_prime_batch])
        # Build the target such that:
        #      - target[selected action] = R + gamma*max(Q(S',:))
        #      - target[other actions] = Q(S,:) (i.e. the loss is 0 for non-selected actions)
        A_batch_oh = one_hot(A_batch, self.A)[:,None,:]
