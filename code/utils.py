...
def temporal_one_hot(indices, length):
    T = indices.shape[1]
    return np.concatenate([one_hot(indices[:,i], length)[:,None,:] for i in range(T)], axis=1)
...
def pad_empty_grids(to_pad, M):
    sh = list(to_pad.shape)
    sh_pad = sh[0:1] + [M-sh[1]] + sh[2:]
    pad = np.zeros(sh_pad, dtype=to_pad.dtype)
    return np.concatenate([pad, to_pad], axis=1)